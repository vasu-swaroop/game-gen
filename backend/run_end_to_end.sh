...
async def create_campaign(name: str, description: str = "", dm_name: Optional[str] = None):
    ...
    campaign = {"id": new_id, "name": name, "description": description, "dm_name": dm_name or "AI DM", 
                "created_at": datetime.utcnow().isoformat(), "state": {"events": []}}
...
async def chat_send(session_id: str, user_message: str, campaign_id: Optional[int] = None):
    ...
    response_text = generate_dm_response(user_message, len(campaigns_db) or 1)
    
    return {"session_id": session_id, "bot_response": response_text, "character_id": campaign_id}
